...
function [percentage_botcontact_area_in_range, area_in, area_range, area_in_range, area_plaat, Points_index, C_list_index, range_index, C_list_range_index] = botcontact_area_range(botmesh, plaatmesh, range)
...
[shortest_distance, closest_bone_points, mindistance] = find_shortest_distance(plaatmesh, botmesh);
...
% hold on 
% %trimesh(C_list_plaat(C_list_index,:), Points_plaat(:, 1), Points_plaat(:, 2), Points_plaat(:, 3), 'FaceColor', 'g', 'EdgeColor', 'r')
% axis equal
% rotate3d('on')
...
%% Voor als je het wil plotten uncomment

% hold on 
% kleur_transparant(botmesh, plaatmesh, []);
%plot3(Points_plaat(range_index, 1), Points_plaat(range_index, 2), Points_plaat(range_index, 3), 'r.', 'MarkerSize', 20, 'LineStyle', 'none') %punten in range
% trimesh(C_list_plaat(C_list_index, :), Points_plaat(:, 1), Points_plaat(:, 2), Points_plaat(:, 3), 'FaceColor', 'green', 'EdgeColor', 'red'); %driehoekjes in bot
% %trimesh(C_list_plaat(C_list_range_index, :), Points_plaat(:, 1), Points_plaat(:, 2), Points_plaat(:, 3), 'FaceColor', 'magenta', 'EdgeColor', 'cyan'); %driehoekjes van range
% axis equal 
% rotate3d('on')
