function callbackClickA3DPoint(~, ~, Points_bot)
...
rotatedPoints_bot = rot * Points_bot;

% the clicked point represented in the view frame
rotatedPoints_bot_Front = rot * point' ;

% find the nearest neighbour to the clicked point 
Points_bot_index = dsearchn(rotatedPoints_bot(1:2,:)', ... 
    rotatedPoints_bot_Front(1:2));

h = findobj(gca,'Tag','pt'); % try to find the old point
selectedPoint =Points_bot(:, Points_bot_index); 
...
assignin("caller", "Points_bot_index", Points_bot_index)
